...
var tooBigString = "Función no compatible con este programa.";
...
function franson() {
  ...
  for (let i = 0; i < parseInt(numVariables.value); i++) {
    ...
    if (isNaN(valorVariable)) {
      alert("Error: Alguna variable no es un número.");
      return;
    }
    ...
    if (i == 0) {
      ...
      fullString += `${
        valorVariable < 0 ? valorVariable : "+" + ` ${valorVariable}`
      }x${i + 1} `;
    }
  }
  ...
  for (let a = 0; a < parseInt(numRes); a++) {
    for (let i = 0; i < parseInt(numVariables.value); i++) {
      var valorVariable = parseFloat(
        document.getElementById(`resx${i + 1}Res${a}`).value
      );
      console.log(valorVariable);
      if (isNaN(valorVariable)) {
        alert("Error: Alguna variable no es un número.");
...
function numeroDeVariables() {
  ...
  limpiar.innerHTML = "";
  var numVariables = document.getElementById("numVariables");
  var content = `<label class="funcionObjt">Función Objetivo</label>`;
  for (let i = 0; i < parseFloat(numVariables.value); i++) {
    content += `<input class="input" type="number" step="any" id="x${
      i + 1
    }"><label>X${i + 1}</label>`;
    if (i < parseFloat(numVariables.value) - 1) {
      content += `<label class="mrml"> + </label>`;
    }
  }
...
function numeroDeRestricciones() {
  ...
  for (let a = 0; a < parseFloat(numRes.value); a++) {
    for (let i = 0; i < parseFloat(numVariables.value); i++) {
      content += `<input class="input" type="number" step="any" id="resx${
        i + 1
      }Res${a}"><label class="mrml">X${i + 1}</label>`;
      content += `<label class="mrml"> + </label>`;
    }
    content += ` <select class="selectOption" id="igualadorX${a + 1}Res${a}">
					  <option value="<="><=</option>
					  <option value=">=">>=</option>
					  <option value="=">=</option>
				  </select>
				  <input class="input" id="resX${a + 1}Res${a}" type="number" step="any"><br>`;
  }
...
function SetupTableau() {
  ...
  if (check == -1) {
    document.theSpreadsheet.expr.value = "Por favor verifica tus datos.";
    document.theSpreadsheet.output.value =
      "Por favor verifica tus datos." + cr + cr;
    okToRoll = false;
    return 666;
  }
...
function displayFinalStatus() {
  ...
  if (TableauNumber > maxSteps)
    document.theSpreadsheet.expr.value = "No se encontró una solución óptima.";
  else if (singular)
    document.theSpreadsheet.expr.value =
      "No existe una solución óptima para este problema";
  else {
...
function doIt() {
  var element = document.getElementById("areaTexto");
  element.classList.remove("primer");
  element.classList.add("segundo");
  franson();
  ...
  // Option 2 // preliminary checks
    ...
    if (okToRoll) {
      ...
      if (document.theSpreadsheet.input.value == "") {
        document.theSpreadsheet.expr.value =
          "Introduzca un problema de programación lineal valido";
        okToRoll = false;
      }
    } // if okToRoll
  } // end of this option
  ...
  //
    ...
    document.theSpreadsheet.expr.value =
      "Presiona el botón para resolver el problema.";
